...
A model is an [interpretable model] if its intepretation is easy. This is the case for most  [mathematical model]s but not all [algorithmic model]s. Other words for an [interpretable model] are **transparent**, **explainable** or **comprehensive** model. 
...
$X$ is used as a generic term for [explanatory variables], [input variables] or [independent variables] in a [regression analysis] but also to indicate a [random] variable.  
...
#### z-scores 
The   z-scores are the [residuals] of a [distributional regression] model. 
