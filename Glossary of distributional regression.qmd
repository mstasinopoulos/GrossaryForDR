...
author: Dimitrios Mikis Stasinopoulos, 
...
This is a glossary of terms and ideas related  to models in general but more specifically to [distributional regression] models. This glossary is based on ideas presented in the talk [Regression Models; how to adapt for climate change challenges](https://mstasinopoulos.github.io/Brazil2025-talk/talk_2.html#/title-slide)]  given by Mikis Stasinopoulos, University of Greenwich, to the XVII Encontro Mineiro de Statistica on Octomber 2025c in Lavras, Brazil, but also from the realization that different data scientist use different terminology according to their background.  Typically the [response] variable a well known expresion   to describe the variable of interest in a [regression analysis] it is called the [target] in [machine learning]. 
...
Additive smoothers occurs when the contribution of one or more continuous [term]s is added to the rest of the terms in the equation of  a model i.e. $s_1(x_1)+s_2(x_2)+ s_3(x_1, x_2)$, where the $s()$s  are different smooth functions

#### additive model

An additive model occurs when the contribution of each [term]s is added to the rest of the terms in the [formula] of  a model. Let $x_1$ abd $x_2$  be two continuous variables and  $f_1$ and $f_2$ to be [factor]s. i.e. $b_1 x_1+s_2(x_2)+ f+ +F*xs_3(x_1, x_2)$, where the $s()$s  are different smooth functions


#### adequate fit

We call a [fit] adequate if the [model] represents the [data] well. Within distributional regression we have two type of adequacy. The first has to do of whether the [distribution] of the [response] is fitted adequately the second on  whether the chosen [term]s are adequate for the [distribution parameters] model. 
...
For ALE see [accumulated local effects] 

#### algorithm 

An algorithm is a step-by-step computational procedure designed to perform a task. For [input-output model]s this task mostly has to do with finding an unknown function say $g()$, which connect the input with the output, i.e. $Y=g(X)$. 
...
Assumptions are axiomatic statements needed to be accepted for the model to work. Models need assumptions because of their simplified nature. The reasoning behind assumption is that if the assumptions are correct then the model can be useful. There are **explicit** and **implicit** assumptions. The **explicit** assumptions  are usually mathematical ones and could be  easily checked using [diagnostics] tools. The  **implicit** assumptions are common in [algorithmic model]s ans they are more difficult to check. **Incorrect** assumptions could lead to questionable scientific discoveries.  For an [agent based  model] the assumptions are the different ways  the agent are behaving.

#### AIC

AIC stands for the Akaike Information Criterion, @Akaike73b. In distributional regression model it is used to  compare different fitted models. The AIC is defined as [deviance] $+2\times df$  where $df$ are the [degrees of freedom] and 2 stands for the **penalty**. See also [GAIC] and [BIC].
...
Bagging refers to [bootstrapping] followed by am [averaging models] procedure.
...
A bucket plot, @de2022bucket, is a residual based diagnostic tool which can detect [skewness] and [kurtosis] in the [residuals] of a [model] and therefore of the [response] variable .  Needs a plot an explanation...
...
Next we show plots of different types of continuous distributions;
...
`data.frame` is the way a [data] set is refereed to in the `R` statistical programming language.   


#### data partition

The term [data partition] has two different meanings.  The fist has to do with [statistical modelling]. Data partition in the process of [statistical modelling] helps the model building process, the model interpretation, the checking for [over-fit] and  also helps to improve the [statistical inference] by providing extra information about variations in the parameters. There different types of data partition;
 a **single** partition of a data set provides holdout samples for prediction and validation purposes. A **multiple** partitions as for example [bootstrapping] and  [K-fold cross validation] help inference. 
   
 ![Diffrent data partitions](data_partition.png){width=500}     


 The second meaning of data partition occurs in modelling. Some models like classification and [regression tree]s try to separate the data in such a way that the resulted subclass are homogeneous.  
...
The [data generating mechanism] is a set of assumptions on how the data were generated. It is often difficult to check  those [assumptions] because of the possible [Rashomon effect] (different assumptions could easily lead to similar results). In distributional regression the data generating mechanism applies mostly on the way the [response] is generated given the [explanatory variables].    
...
The [deviance] is a measure of goodness of fit and defined as $-2 \ell(\theta)$, that is, minus twice the [log likelihood]. It can be used for comparison between models. It can be defined as a *training* or as a *test* deviance depending whether is evaluated at the [training] or [test] data sets. In GAMLSS modelling the `training deviance`, can be used together with its penalised form the Generalise Akaike Information Criterion, `GAIC`,  for model comparison. The deviance is an [empirical risk] measure based on `information criterion` concepts. It is a `summary statistics`, because it is a sum of the individual deviance increments and therefore an overall measure of goodness of fit. It tell us how well the conditional distribution of $f(y|X)$ fits the data overall. It does not tells whether the tail or the middle of the distribution fits well. The deviance increments may help to obtain such information.

#### density function

At a theoretical level a density function is the [pdf] of a random variable. At the sample level a density function of a vector in the data is an empirical distribution function of a variable in the data.   

#### diagnostics 

Diagnostics are (usually graphical) tools for helping to check the [assumption]s of the model.


#### discrete distribution 

A [discrete distribution] is distribution function [pdf] of a random veriable which takes only integer values.
The only property that any  discrete distribution  must have is $\sum(i {R(y)} f(y_i)$

#### distribution

The term [distribution] refer to a probability distribution function  [pdf], i.e. $D(y | \boldsymbol{\theta})$. The only property which is required for any [pdf] is that it should sum up to one.  Within a distributional regression set-up the [pdf] affects the behaviour of the response variable. 


#### distribution parameters

Theoretical distributions depend on distribution parameters $\boldsymbol{\theta}$ where $\boldsymbol{\theta}$ is a [vector] of length $K$. The more distribution parameters exist the more flexible the distribution is. For example in one parameter distribution like, for example the exponential distribution, the location, ....  @Rigbyetal2019  
...
The empirical [cdf]  is the cumulative function function of any variable in the [data] ([sample]). The Empirical cdf  is a step function that shows the proportion of observed data points less than or equal to a specific value.It creates a cumulative probability distribution directly from a vetron in he data, without assuming a  theoretical model therefore is the source of a mavy of non-parametric procedures in statistics. Any [vector] in a [tabular data] set can have an empirical cdf. For a given value $x_o$ of the any continuous variable in the data $\textbf{x}$ the empirical cdf gives the fraction of data points in $\textbf{x}$  i.e, #$(\textbf{x} \le x_o)$. 
...
library(gamlss.data)
library(gamlss2)
m1 <- gamlss2(rent~s(area)+s(yearc)+location+kitchen+kitchen|s(area)+s(yearc), family=BCTo, data=rent99 )
library(gamlss.ggplots)
gg <- resid_ecdf(m1)
...
The term [error] in a [model] is what chaharacterised a modes to  [stochastic], that is, it the model conails a set of  [probability] elements. In [regression analysis] the [error] is ofter a terms used to describe the residual variation of a model. 
...
#### GLM
 ...
 GLM stands for  Generalized Linear Model, @NelderWeddeburn72, a [mathematical model] which dominated the 1980's. As [input-output model] it can be written as $$
    ...
    $$ 
  where $E(Y)$ is the expected values of the output $Y$ and $f()$ is a general function connecting the $X$'s with the expected value of $Y$.
...
We refer to the function  $g()$ as an true relationship between $X$ and $Y$ in [input-output model] which is unknown and  which the [model] tries to approximate.
...
While the above definion of IT residuals works perfectly for [continuous distribution]s are not ideal for [discrete distribution]s especially if the range of the response is limited to small range i.e. $0,1,\ldots,10$. The same applies to [mixed distribution]s where there is small amount of discrete values. A randomisation of the PIT residuals between the values $y_i$ and $y_i+1$ usually solves the problem see for example Chapter 10 of @Stasinopoulosetal2017.
...
A [risk] function is a function containing both the [data] and the [fit]ted [model]. It is measure the average discrepancy between the assumed model amd the actual data.   
...
The term [statistic] refers to a functin of the [sample] ([data]). For example, the mean of a sample is a statistic $\bar{x}= \frac{1}{n}\sum_{i=1}^{n} x_i$. Stastistcs are used to calculated characteristics of the the teoretical distribution assumed for the random variable $X$. For example $\bar{x}$ can be ussed as estimate of the mean $E(X)$ of the theoretical distribution assumed for $x$ i.e. $E(X)= \int_{-\infty}^{\infty} X f(X) dx$, where $f(X)$ is the assumes ditribution for the [vector] $x$.  

#### statistical inference 

Is the procees of drawing infrerence from the [sample] to the [population], from going to a specific case to the general situation. The path from to the spefic to the general can only be acheived using [assuptions] and therefore is very treacherous ans irrelevant if the assumtions are not right.    
...
A [smooth function] is a term in a [model].  Most smooth functions can be written a  [stochastic] model as  $\textbf{y}= \textbf{S}  \boldsymbol{\gamma}+  \boldsymbol{{\ephsilon}$

#### smooth model

A smooth model is a model containing smooth functions.   

#### smoother 

A smoother is a smooth [term] in the equations of a model i.e. `s(x)`. 
...
The validation set is used for tuning the model, (estimating its [hyper parameter]s) 
