...
For more details see pp. 521-522   of @Rigbyetal2019. The expected value of the distribution is $E(Y)=np$ and the variance is $Var(Y)=np(1-p)$. @fig-binomial show the three different binomial distributions with binomial denominator equal to 10 and different probabilities at   
...
The [Bernoulli distribution] is special case of the [binomial distribution] when $n=1$. A Bernoulli [random] variables has range of values  $0$ or $1$, expected value $E(Y)=p$ and variance $p(1-p)$.
@fig-bernoulli show three realization of the [Bernoulli distribution] at different values of $p=(0.1,0.3,0.5)$
SOMETHING IS WRING WITH IT the p should be in the first line not the second
...
Data partition in the process of [statistical modelling] helps the model building process, [model] [interpretation], the checking for [over fit] and  also helps to improve the [statistical inference] by providing extra [information] about variations in the parameters. There are  different types of data partition;
 ...
 The second meaning of [data partition] tries to answer the following question. Is there a way to partition the data in such a way that the relationship between the variables in the data data behave differently?; or more generaly can I divide the data in such way that different models could apply to different parts of the [data]?.  Models like [regression tree]s try to separate the data in such a way that the resulted subclass are homogeneous by fitting differenr constant models. $\ldots$ 
...
The term [pacf] stands for partial [autocorrelation] function. The [pacf] is a [diagnostic] tool for detecting [autocorrelation] in a [vector] $\text{x}$. 
