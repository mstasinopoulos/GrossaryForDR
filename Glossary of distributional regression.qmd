...
Note that most [model]s analysing [data] use mathematics to model the relationships between variables. For example, in [distributional regression] each of the [parameters] of the [distribution] is a function of the [explanatory variables], e.g. $$\boldsymbol{\theta}_i= G(\textbf{x}_1, \textbf{x}_2, \ldots, \textbf{x}_r).$$ The task of a [model] is to estimate $G()$. Subsequently we will use the definition of a [mathematical model] as a model in which the interpretation of $G()$ is simple and the definition of an [algorithmic model] as one in which the the interpretation of $G()$ is difficult. Our definition is in line to @breiman2003statistical distinction between [data modelling] and [algorithmic modelling] but we did not like [data modelling], because it is implies any [model] involving [data]. We use instead the distinction between [mathematical model]s and [algorithmic model]s. If in addition a [model] contains a [stochastic] part, that is, it relies on [probability] statements, the we have a [statistical model] or \[stocastic model\[\].

Any [statistical model] needs an [objective measure], a function describing the **distance**, between the [data] and the [model]. We would divide the [objective measure] in two components accord to in which [data] are used to evaluate it;

-   [training measure]: a [goodness of fit] [measure] evaluated at the [training] [data], and used at the [model fitting] stage to [fit] the model;

-   [test measure]: a [goodness of fit] [measure] evaluated at the [test] [data], for [model comparison] and [prediction] performance.

Note that the same [objective measure] can be used for both the [training] and the [test] data, when [data partition] is involved. Both the [test] and [training] [measure]s should be relevant to the [questions]. For example, if the interested lies in the tail behaviour of the [response] we should never use an [test] or [training] [measure] concentrating at the centre of the distribution of the [response] e.g. [MSE].
...
After a [model selection] procedure an [adequate model] is selected, and the [interpretation] of the model begins to help answering the [questions]. Using a [model selection] procedure to find an [adequate model] is the essence of any [statistical modelling] process.
